...
public interface TermFixpoint<P> extends Term.Visitor<P, @NotNull Term> {
  ...
  @Override default @NotNull Term visitDataCall(@NotNull CallTerm.Data dataCall, P p) {
    ...
    var sortArgs = ArrayUtil.map(dataCall.sortArgs(), sort -> visitSort(sort, p));
    if (ArrayUtil.identical(dataCall.sortArgs(), sortArgs)
  ...
  @Override default @NotNull Term visitConCall(@NotNull CallTerm.Con conCall, P p) {
    ...
    var sortArgs = ArrayUtil.map(conCall.sortArgs(), sort -> visitSort(sort, p));
    if (conCall.head().dataArgs().sameElements(dataArgs, true)
      && ArrayUtil.identical(sortArgs, conCall.sortArgs())
  ...
  @Override default @NotNull Term visitStructCall(@NotNull CallTerm.Struct structCall, P p) {
    ...
    var sortArgs = ArrayUtil.map(structCall.sortArgs(), sort -> visitSort(sort, p));
    if (ArrayUtil.identical(structCall.sortArgs(), sortArgs)
  ...
  @Override default @NotNull Term visitFnCall(CallTerm.@NotNull Fn fnCall, P p) {
    ...
    var sortArgs = ArrayUtil.map(fnCall.sortArgs(), sort -> visitSort(sort, p));
    if (ArrayUtil.identical(fnCall.sortArgs(), sortArgs)
  ...
  @Override default @NotNull Term visitPrimCall(CallTerm.@NotNull Prim prim, P p) {
    ...
    var sortArgs = ArrayUtil.map(prim.sortArgs(), sort -> visitSort(sort, p));
    if (prim.args().sameElements(args, true)
      && ArrayUtil.identical(prim.sortArgs(), sortArgs)) return prim;
